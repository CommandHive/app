...
const MyServersPage = () => {
  ...
  const generateChartData = (sessionId: string) => {
    ...
    for (let i = 6; i >= 0; i--) {
      ...
      data.push({
        day: i === 0 ? 'Today' : i === 1 ? 'Yesterday' : date.toLocaleDateString('en-US', { weekday: 'short' }),
        totalCalls: Math.max(0, baseCalls + Math.floor(Math.random() * 20) - 10),
        invocations: Math.max(0, baseInvocations + Math.floor(Math.random() * 25) - 12)
      })
    }
            ...
            {sessions.length === 0 ? (
              ...
              <div className="w-[1216px] grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {sessions.map((session) => {
                  ...
                  const totalCalls = chartData[chartData.length - 1]?.totalCalls || 0
                  const totalInvocations = chartData[chartData.length - 1]?.invocations || 0
